" rename 栈：new_name.txt 第 2 行（Eve 修改过）Tab 入栈后显示旧路径 blame
...
call assert_equal(3, len(s:records), '入栈后应显示父版本（3 行）')
call assert_notequal(s:orig_buf, winbufnr(s:fw), '源窗口应切到历史缓冲区')
call assert_match('old_name', bufname(winbufnr(s:fw)),
            \ '历史缓冲区应基于旧文件名创建')
" 修改行的父版本行内容应相同（行号正确映射的旁证）
call assert_equal('r line 2', get(getbufline(winbufnr(s:fw), 2, 2), 0, ''),
            \ '父版本第 2 行应为修改前的内容')
